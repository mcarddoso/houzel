...
import { ArrowUp, Atom, AudioLines, Check, ChevronDown, ChevronsUpDown, Copy, FileText, Loader, Mail, Paperclip, PenLine, SquareArrowDown, X } from "lucide-react";
...
export default function Home() {
    ...
    async function handleGenerate() {
        ...
        const userMsg: Msg = { role: "user", content: essay };
        const nextMessages = [
          ...messages,
          userMsg,
          ...attachments.map(att => ({
            ...
            content: JSON.stringify({
              filename: att.name,
              mimetype: att.data.split(";")[0].replace("data:", ""),
              base64: att.data.split(",")[1],
            })
          }))
        ];
        ...
        if (Array.isArray(json.messages)) {
          ...
          console.error("API returned unexpected payload:", json);
        }
    ...
    return (
        <>
            ...
            />
            <main className="flex min-h-screen flex-col w-full items-center justify-start h-full bg-[var(--background-gray-main)]">
